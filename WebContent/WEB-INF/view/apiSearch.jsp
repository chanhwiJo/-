...
<%
	...
	String search_course_name = CmmUtil.nvl((String)request.getAttribute("course_name"));
	
	String allurl = url + servicekey + "xml/" + "SeoulGilWalkCourse/" + first + "/" + last + "/" + search_course_name;
	Document text = Jsoup.connect(allurl).get();
	Elements item = text.select("row");
	
	Elements totalCount = text.select("list_total_count");
	int count = 0;
	if (totalCount.text().equals("")) {
		count = 0;
	} else {
 		count = Integer.parseInt(totalCount.text());
	}
	...
	if (endPage == Integer.parseInt(pageNo)) { // endPage 표현하기 위한 if문
		countList = item.size();
	}
	
	String[] cpi_idx = new String[countList];
	String[] course_name = new String[countList];
	String[] cpi_name = new String[countList];
	String[] area_gu = new String[countList];
						...
						<div class="col-md-12">
							<div class="card">
								...
								<div class="card-body">
									...
									<div id="paging">
										<nav aria-label="Page navigation example">
											<ul class="pagination justify-content-center">
												<%if(startPage != 1) {%>
												<li class="page-item">
													<a class="page-link" href="/apiSearch.do?COURSE_NAME=<%=search_course_name%>&no=<%=((pageno -11) / 10) * 10 + 1%>" aria-label="Previous">
												...
												<%for (int iCount = startPage; iCount <= endPage; iCount++) {
													...
													<%}else{ %>
														<li class="page-item"><a class="page-link" href="/apiSearch.do?COURSE_NAME=<%=search_course_name%>&no=<%=iCount%>"><%=iCount%></a></li>
													<%} }%>
												<%if(endPage != totalPage) {%>
												<li class="page-item">
													<a class="page-link" href="/apiSearch.do?COURSE_NAME=<%=search_course_name%>&no=<%=((pageno +9) / 10) * 10 + 1%>" aria-label="Next">
									...
									<%}else{ %>
									<div align="center" style="color:red;">'<%=search_course_name %>' 에 해당하는 검색결과가 없습니다.</div>
									<%} %>
								</div>
							</div>
						</div>
